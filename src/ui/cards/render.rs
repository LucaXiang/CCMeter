...
/// Render the Codex per-model breakdown (Codex source view). Codex has no
/// ProjectGroup, so this stands in for the normal card/detail rendering: a
/// header with a dynamic per-model legend, a per-model list (tokens + cost +
/// cost share), and a stacked-by-model daily cost chart.
pub fn render_codex_breakdown(
    frame: &mut Frame,
    area: Rect,
    breakdown: &crate::app::CodexBreakdown,
    range_start: NaiveDate,
    range_end: NaiveDate,
) {
    if area.height < 3 || area.width < 24 || breakdown.rows.is_empty() {
        return;
    }
    let t = theme();
    let total_cost: f64 = breakdown.rows.iter().map(|(_, _, c)| *c).sum();

    // Header + dynamic legend (one swatch per model actually present).
    ...
    for (label, _, _) in &breakdown.rows {
        header.push(Span::styled("■", Style::default().fg(t.model_color(label))));
    ...
    // Per-model list: swatch · name · tokens · cost · cost share.
    let n = breakdown.rows.len() as u16;
    let list_h = n.min(area.height.saturating_sub(2));
    for (i, (label, tokens, cost)) in breakdown.rows.iter().enumerate() {
        let row = i as u16;
        if row >= list_h {
            break;
        }
        ...
        let line = Line::from(vec![
            Span::styled("■ ", Style::default().fg(t.model_color(label))),
            Span::styled(format!("{label:<16}"), Style::default().fg(t.text_primary)),
            Span::styled(
                format!("{:>9} ", format_tokens(*tokens)),
                Style::default().fg(t.tokens_in),
            ),
            Span::styled(
        ...
        frame.render_widget(
            Paragraph::new(line),
            Rect::new(area.x + 1, area.y + 1 + row, area.width.saturating_sub(1), 1),
        );
    }

    // Stacked-by-model daily cost chart below the list.
    let chart_y = area.y + 2 + list_h;
    if chart_y + 3 <= area.y + area.height {
        let chart_area = Rect::new(area.x, chart_y, area.width, area.y + area.height - chart_y);
        let chart_w = chart_area.width.saturating_sub(7) as usize;
        if chart_w >= 2 {
            let series: Vec<(String, Vec<f64>)> = breakdown
                .daily
                .iter()
                .map(|(label, data)| {
                    (
                        label.clone(),
                        bucket_f64_series(data, range_start, range_end, chart_w),
                    )
                })
                .collect();
            let x_labels = x_axis_labels(range_start, range_end, chart_w);
            render_model_stacked_chart(frame, chart_area, &series, format_y_cost, &x_labels);
        }
    }
}
